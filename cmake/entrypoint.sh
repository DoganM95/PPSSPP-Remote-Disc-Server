...
update_interval=5 # The higher the interval, the faster it updates the lib on change, but also increases cpu usage
...
function create_gamelist() {
    sed -i "s|MaxRecent = .*|&\nFileName$file_counter = $1|g" $ppsspp_ini_file # Add game to ppsspp-ini recents
